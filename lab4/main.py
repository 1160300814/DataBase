...
def write_relation(relation, required_blk, addr):
    for i in range(required_blk):
        ...
        if not buffer.writeBlockToDisk(addr, blk_num): # 写入
...
def write_data(data_set, required_blk, tuple_number, addr):  # 写入数据，如结果
    ...
    number_of_written = (required_blk - 1) * tuple_number  # 已经写入磁盘的数据个数,写最后一块，最后一块可能不满
...
class RelationSelectionAlgorithm:
    ...
    def selection_linear(relation, attribute, value, addr):
        ...
        relation_start_blk_number = blk_dict[relation]  # 首先找到关系的起始文件块号
        ...
        while True:
            ...
            for i_ in range(int(number_of_tuple)):
                if int(read_data[i_ * 2 + attr_index]) == value:
                    ...
                    print(relation, ":", read_data[i_ * 2], read_data[i_ * 2 + 1])
        ...
        addr = write_data(data, int(ceil(len(data) / 7.0)), 7, addr)   # ceil()方法返回x的值上限
        return addr
    ...
    def selection_binary_search(relation, attribute, value, blk_num):
        ...
        while True:
            ...
            sort_data = sorted(sort_data, key=lambda t: t[attr_index])  # 排序
            ...
            while search_low <= search_high:
                ...
                if sort_data[search_middle][attr_index] == value:
                    ...
                    print(relation, ":", sort_data[search_middle][0],  sort_data[search_middle][1])
        ...
        blk_num = write_data(data, int(ceil(len(data) / 7.0)), 7, blk_num)   # ceil()方法返回x的值上限
        return blk_num

    def sort_for_index(self, relation, index_table, attr_index, blk_number):  # 排序，然后键索引
        data = []
        # 首先找到第一个关系的起始文件块号
    ...
    @staticmethod
    def create_index(data, attr_index, index_table, blk_number):  # 建索引
        index = 0
        while index < len(data):
            if index + 7 < len(data) and data[index][attr_index] < data[index + 7][attr_index]:
                ...
                index_table.append([data[index][attr_index], blk_number])  # 剩余数据不会多余一块的量
                blk_number += 1
                break
        return index_table, blk_number
    ...
    def selection_index(self, relation, attribute, value, blk_number):
        ...
        present_blk_number = hit_blk_number  #
        data = []  # 保存满足条件的元组，会写入到文件块中
        write_blk_index = buffer.getNewBlockInBuffer()  # 为选择的数据申请一个缓冲区
        if write_blk_index == -1:
            print("缓冲区已满，不能为选择的数据申请一个缓冲区")
    ...
    def project(relation, attribute, blk_number):
        ...
        while True:
            ...
            for i in range(int(number_of_tuple)):
                ...
                print(relation, ":", read_data[i * 2 + attr_index])
...
class JoinOperationAlgorithm:
    ...
    def nest_loop_join(relation_first, relation_second, blk_numbers_of_first, blk_numbers_of_second,
                       blk_number):
        if blk_numbers_of_first > blk_numbers_of_second:  # 内表为大表，外表为小表
        ...
        while i < len(first_data) or j < len(second_data):
            if i < len(first_data) and j < len(second_data):
                if first_data[i][0] == second_data[j][0]:
                    ...
                    print(relation_first, '的元组', first_data[i][0], first_data[i][1],
                          '和', relation_second, '的元组', second_data[j][0], second_data[j][1], '连接')
                    ...
                    while temp_index < len(second_data) and first_data[i][0] == \
                        ...
                        print(relation_first, '的元组', first_data[i][0], first_data[i][1],
                              '和', relation_second, '的元组', second_data[temp_index][0], second_data[temp_index][1], '连接')
                        temp_index += 1
            ...
            for j_ in range(len(bucket_of_r[i])):
                for k_ in range(len(bucket_of_s[i])):
                    if bucket_of_r[i][j_][0] == bucket_of_s[i][k_][0]:
                        ...
                        print(relation_first, '的元组', bucket_of_r[i][j_][0], bucket_of_r[i][j_][1], '和',
                              relation_second, '的元组',
                              bucket_of_s[i][k_][0], bucket_of_s[i][k_][1], '连接')
...
if __name__ == '__main__':
   ...
   # blk_number = SetOperationAlgorithm.set_operate('R', 'S', 'except', blk_number)
    ...
    blk_dict['nest-loop-join'] = blk_number
